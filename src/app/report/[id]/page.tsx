...
async function getReportData(id: string) {
  try {
    ...
    await db.reportView.create({
      data: {
        checkId: check.id,
        viewerIp: '', // Would be populated from request headers
        userAgent: '', // Would be populated from request headers
        referrer: '', // Would be populated from request headers
        country: '' // Would be populated from geo IP
      }
    })
                ...
                <div className="flex items-center justify-between">
                  ...
                  <Button 
                    onClick={() => {
                      if (navigator.share) {
                        navigator.share({
                          title: 'LinkShield Security Report',
                          text: `Security analysis for ${report.url}`,
                          url: shareUrl
                        })
                      } else {
